...
if uploaded_files:
    ...
    for file_idx, uploaded_file in enumerate(uploaded_files):
        ...
        # Create expander for each image (useful for batch processing)
        with st.expander(f"📷 Image {file_idx + 1}: {uploaded_file.name}", expanded=len(uploaded_files) == 1):
    ...
    with st.expander("💡 How to use this application"):
        st.markdown("""
        1. **Upload Image(s)**: Click the upload button and select one or more dental X-ray images
