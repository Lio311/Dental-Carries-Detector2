...
with st.sidebar:
    ...
    with st.expander("📊 Model Information"):
        ...
        st.write("**mAP50:** 82.6%")
        st.write("**Precision:** 86.2%")
        st.write("**Recall:** 75.7%")
    ...
    try:
        ...
        for box in results[0].boxes:
            ...
            if score >= conf_threshold:
                ...
                # Create semi-transparent overlay for the detected area
                overlay = annotated_img.copy()
                cv2.rectangle(overlay, (x1, y1), (x2, y2), color, -1)  # Filled rectangle
                cv2.addWeighted(overlay, 0.3, annotated_img, 0.7, 0, annotated_img)  # 30% opacity
                
                # Draw border around the area
                cv2.rectangle(annotated_img, (x1, y1), (x2, y2), color, 2)
                
                # Create circular badge for confidence percentage
                label_text = f"{score:.0%}"
                
                # Calculate badge position (top-right corner of detection)
                badge_radius = 25
                badge_center_x = x2 - badge_radius - 5
                badge_center_y = y1 + badge_radius + 5
                
                # Draw filled circle for badge background
                cv2.circle(annotated_img, (badge_center_x, badge_center_y), badge_radius, color, -1)
                
                # Draw white border around badge
                cv2.circle(annotated_img, (badge_center_x, badge_center_y), badge_radius, (255, 255, 255), 2)
                
                # Calculate text size and position for centering
                (text_w, text_h), baseline = cv2.getTextSize(label_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
                text_x = badge_center_x - text_w // 2
                text_y = badge_center_y + text_h // 2
